# Astro Vault 🌱

A Digital Garden template powered by **Astro** and **Obsidian**.
Designed to publish your notes as a static website with zero friction.

## 🚀 Features

* **Obsidian Integration:**
  * ✅ Wikilinks support (`[[Note]]` -> `/note`).
  * ✅ Local Images support (`![[image.png]]` -> `/assets/image.png`).
  * ✅ Auto-Sync: Images are automatically copied to `public/assets` during build.
* **Tech Stack:**
  * **Astro v5** (Content Layer API).
  * **Tailwind CSS** (Planned).
  * **GitHub Pages** (Auto-deploy via Actions).
* **Architecture:**
  * Source of Truth: `src/pages/` (Your Obsidian notes).
  * Build Output: Static HTML/CSS.

## 🛠️ Setup

1. **Install Dependencies:**

    ```bash
    npm install
    ```

2. **Run Development Server:**

    ```bash
    npm run dev
    ```

3. **Clean Cache (if things get weird):**

    ```bash
    npm run clean
    ```

## 📝 Workflow

1. Write your notes in Obsidian inside `src/pages/`.
2. Use standard Wikilinks and drag-and-drop images.
3. Commit and Push to GitHub.
4. GitHub Actions will build and deploy your site automatically.

## 📂 Project Structure

```text
/
├── public/
│   └── assets/  # Generated by sync-assets (Ignored by Git)
├── src/
│   ├── assets/       # YOUR ASSETS GO HERE
│   ├── integrations/  # Custom Astro Integrations (Sync)
│   ├── layouts/       # Page Layouts
│   ├── pages/         # YOUR NOTES (converted to pages) GO HERE
│   ├── plugins/       # Remark/Rehype Plugins
│   └── styles/        # Global CSS
└── astro.config.mjs   # Configuration
```
